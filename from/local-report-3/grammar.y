...
%type <node> item_list item function const_var name arg_list arg_list_ arg expr_list expr if expr_op func_call var_ref number
...
item      : function {
          ...
          ;
function  : KW_LET name arg_list '=' item {
                $$ = Node_new(FUNCTION);
                Node_append_child(&$$, $2);
                Node_append_child(&$$, $3);
          ...
          | KW_LET name arg_list '=' '{' item_list '}' {
                $$ = Node_new(FUNCTION);
                Node_append_child(&$$, $2);
                Node_append_child(&$$, $3);
                Node_append_child(&$$, $6);
...
name      : K_IDENT {
          ...
          ;
arg_list  : '(' ')'       { $$ = Node_new(ARG_LIST); }
          | arg_list_     { $$ = $1; }
          ;
arg_list_ :               { $$ = Node_new(ARG_LIST); }
          | arg_list_ arg {
                Node_append_child(&$1, $2);
                $$ = $1;
            }
